...
// 
// En esencia, actúa como el puente entre el "cerebro" de la aplicación (los datos y la lógica de negocio) y la experiencia del usuario final.
(function(App) {
    ...
    App.ui.general = {
        ...
        showDiscreetMessage: function(message) {
            const container = document.getElementById('discreetMessagesContainer');
            if (!container) return;
            ...
            const msgElement = document.createElement('div');
            msgElement.className = 'discreet-message';
            msgElement.textContent = message;
            container.appendChild(msgElement);
        ...
        initListeners: function() {
            ...
            App.events.on('showDiscreetMessage', (message) => this.showDiscreetMessage(message));
            ...
            document.addEventListener('click', (e) => {
                if (!e.target.closest('.form-container, .category-wrapper, #addCategoryFormContainer, .habit-card, .modal-content, .floating-button, .discreet-message, .date-display-container, .challenge-card')) {
                    document.querySelectorAll('.form-container.active').forEach(f => f.classList.remove('active'));
