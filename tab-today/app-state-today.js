...
(function(App) {
    ...
    function addQuickTask({ name, points = 0, categoryId = null, targetDate = null }) {
        ...
        // ⭐ Asignar peso usando la función centralizada (sin hora = peso máximo)
        App.state.calculateAndAssignMissionWeight(newMission, null);
        ...
        // ⭐ La tarea aparecerá al principio automáticamente por su peso alto
        // No es necesario mover manualmente ni guardar orden
        ...
        deleteTodayTask: function(taskId, skipConfirm = false, targetDate = null) {
            const performDelete = () => {
                ...
                if (taskIndex !== -1) {
                    ...
                    // ⭐ Si la tarea tiene un timer activo, detenerlo
                    if (App.focusTimer && App.focusTimer.hasActiveTimer && App.focusTimer.hasActiveTimer(taskId)) {
                        console.log(`🛑 Deteniendo timer de tarea eliminada: ${taskName}`);
